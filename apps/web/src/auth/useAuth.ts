...
const suffix = () => networkChoice().suffix
const TOKEN_KEY = () => `nimble.jwt${suffix()}`
const ADDRESS_KEY = () => `nimble.address${suffix()}`
const REFRESH_KEY = () => `nimble.refresh${suffix()}`

// One-time key migration from the NIMblink era so nobody gets logged out.
for (const [oldKey, newKey] of [
  ['nimblink.jwt', 'nimble.jwt'], ['nimblink.address', 'nimble.address'], ['nimblink.refresh', 'nimble.refresh'],
] as const) {
...
export function useAuth() {
  ...
  const api = useMemo(() => {
    ...
    const a: Api = new Api(
      networkChoice().base || (import.meta.env.VITE_API_URL ?? 'http://localhost:3000'),
